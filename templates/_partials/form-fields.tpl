...
{else}
  <div class="form-group row no-gutters{if !empty($field.errors)} has-error{/if}">
    <label class="col-md-2 form-control-label{if $field.required} required{/if}">
      {if $field.type !== 'checkbox'}
        {$field.label} :
      {/if}
    </label>
    <div class="col-md-6{if ($field.type === 'radio-buttons')} form-control-valign{/if}{if ($field.type === 'birthday')} hidden-comment{/if}">
